...
from yolo5v3 import Vision, Modality
...
class Control:
    ...
    def set_modality(self, mode):
        ...
        if mode == Modality.Following:
            ...
            self.controller.set_jog_vel_params(1, 1, 10)
            self.controller.set_jog_vel_params(2, 1, 10)
    ...
    def follow(self):
        ...
        if  not self.first_catch:
            ...
            for axes in range(2):
                if abs(error_stage[axes]) > (self.center_tolerance + 1*0.008 + 5*0.008**2) :
    ...
    def main(self):
        # self.initialization([15, 25, 12.4])
